...
def read_data_set():
    ...
    with open(file) as f, open('data_set/missing.csv', 'w') as csvfile:
        ...
        filewriter.writerow(['RNO', 'NAME', 'CLASS', 'GENDER','CGPA','M_MARK','PPG','AT','CT','TT'])
        for row in reader:
            #print row
            t1 = row['RNO']
            t2 = row['NAME']
            ...
            if ((t1=="")or(t2=="")or(t3=="")or(t4=="")or(t5=="")or(t6=="")or(t7=="")or(t8=="")or(t9=="")or(t10=="")):
                ...
                filewriter.writerow([t1,t2,t3,t4,t5,t6,t7,t8,t9,t10])
...
missing_values_data_main.title(sample_data.title)
missing_values_data_main.resizable(False, False)
missing_values_data_main.configure(background=sample_data.background)
########################################################### Element design


message = Label(missing_values_data_main, text=sample_data.titlec,bg=sample_data.background,fg="#003366", width=35,height=3, font=('times', 30, 'italic bold '))
